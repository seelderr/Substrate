...
namespace Substrate.BlockEntities
{
    public class BlockEntitySporePaper : BlockEntityDisplayCase
    {
        ...
        internal bool OnInteract(IPlayer byPlayer, BlockSelection blockSel)
        {
            ...
            SoundAttributes sound = slot.Itemstack?.Block?.Sounds?.Place ?? new SoundAttributes(new AssetLocation("sounds/player/build"), true);
            Api.World.PlaySoundAt(sound, byPlayer, byPlayer, 0.5f);
        ...
        private bool TryTake(IPlayer byPlayer, BlockSelection blockSel)
        {
            if (blockSel.SelectionBoxIndex == 4 || Done)
            {
                ...
                if (slot != null)
                {
                    ...
                    {
                        SoundAttributes sound = stack.Block?.Sounds?.Place ?? new SoundAttributes(new AssetLocation("sounds/player/build"), true);
                        Api.World.PlaySoundAt(sound, byPlayer, byPlayer, 0.5f);
                    }
